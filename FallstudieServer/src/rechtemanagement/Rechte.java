...
public class Rechte {
	...
	// Methode für den ersten Login. gibt 1 zurück wenn der Benutzer sich
	// anmelden darf.
	// 2, falls es den Benutzer nicht in der DB gibt
	// 3, falls der Benutzer gesperrt ist
	// 4, falls das Passwort einfach nicht mit dem Passwort in der Datenbank
	// übereinstimmt.
	...
	// Mehtode beim Überprüfen für die Rechte. Einfach nur zur Sicherheit noch
	// einmal überprüfen,
	...
	public boolean vorgangMoeglich(String benutzername, String Passwort,
		...
		if (login(benutzername, Passwort)) {
			if (Vorgang == alleBenutzer) {
				...
				int LeiterRechte = dbZugriff
						.getBerechtigungzuLeitername(benutzername);
				int MitarbeiterRechte = dbZugriff
						.getBerechtigungzuMitarbeiter(benutzername);
	...
	// Wird für den Client benötigt, damit er weiß welche Seiten er anzeigen
	// darf.
	public List<Integer> erlaubteAnzeigen(String benutzername) {
		...
		if (LeiterRechte == 2 || MitarbeiterRechte == 2) {
			// Der Bereichsleiter darf nicht stricheln.
			Benutzer benutzer = dbZugriff
					.getBenutzervonBenutzername(benutzername);
			if (dbZugriff
					.getOrgaEinheitZuidOrgaEinheit(benutzer.getAktuelleOE())
					.getOrgaEinheitTyp().equals("Bereich")) {
				//Falls die OrgaEinheit ein Bereich ist, wird das Stricheln nicht erlaubt
				//wenn der benutzer leiter ist.
				if (!benutzer.isLeiter())
					rueckgabe.add(1);
			} else
				rueckgabe.add(1);
		}
		if (LeiterRechte == 3 || LeiterRechte == 4 || MitarbeiterRechte == 3
				|| MitarbeiterRechte == 4) {
			rueckgabe.add(2);
		}
		return rueckgabe;
